...
(async()=>{
 ...
 await page.locator('[data-action="goto-slide"][data-page="16"]').click();assert.match(await page.locator('.slide-box img').getAttribute('alt'),/Attention/);
 await page.locator('[data-action="select-material"][data-id="intro"]').click();assert.equal(await page.locator('.thumb').count(),2);
 await page.locator('[data-action="select-material"][data-id="main"]').click();await page.locator('[data-action="goto-slide"][data-page="12"]').click();
 ...
 await page.locator('[data-action="open-chat"]').first().click();await page.locator('#chat-input').fill('Vì sao đầu ra luôn là một phân bố xác suất?');await page.locator('#chat-form button').click();
 await page.locator('.chat-message.assistant').waitFor({timeout:45000});
 ...
 await page.locator('#query').fill('phan bo xac suat');await page.locator('.card').first().click();await page.locator('[data-action="source"]').first().click();assert.equal(await page.locator('#page-number').inputValue(),'12');
